...
- переподключение камеры при сбое (состояние в статусбаре, без перезапуска приложения)
...
7. **Нет перезапуска камеры** при сбое — было: `camera_error` → модальный бокс и по сути мёртвое приложение. **Решено**: `CameraWorker` уходит в цикл переподключения (ретрай каждые 2 с, поиск по нескольким источникам: последний рабочий → индексы 0..5 → `/dev/video*` на Linux, каждый валидируется пробным кадром), состояние выводится в статусбар (`camera_error` один раз за сбой, `camera_recovered` при успехе).
